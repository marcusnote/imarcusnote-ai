...
export async function settleExpiredMonthlyMp(memberId, member) {
  ...
  // 유료 플랜이 아닌 경우는 기존 보수 원칙대로 0 처리
  const isPaidPlan = ["basic", "standard", "premium", "elite"].includes(
    currentPlanKey
  );

  if (!isPaidPlan || monthlyMp <= 0) {
    return await zeroOutExpiredMp(memberId, member);
  }

  const carryoverMp = computeCarryoverMp(member, MP_ROLLOVER_RATE);
  const nextRemainingMp = monthlyMp + carryoverMp;
  ...
  const patch = buildMpPatch(member, {
    ...
    mp: nextRemainingMp,
    carryover_mp: carryoverMp,
    carryover_expires_at: nextResetAt,
