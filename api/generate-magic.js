...
/* =========================================================
 * v8.9.2 INTEGRATED SYNC + DIVERSITY PATCH
 * - precise append-safe override for local formatMagicResponse
 * - preserves exactly 25 pairs
 * - reduces mismatch between questions and answers
 * - improves chapter-specific fallback control and diversity
 * ========================================================= */
(function apply_v892_integrated_sync_patch() {
  const PATCH_TAG = "v8.9.2-integrated-sync";
  console.log(`[${PATCH_TAG}] loading...`);
  ...
  function stripNumberPrefix(s) {
    return normalizeLine(String(s || "").replace(/^\d+[.)]\s*/, ""));
  }

  function toPairArray(formatted = {}) {
    const qLines = String(formatted?.questions || "")
      .split("\n")
      .map((s) => stripNumberPrefix(s))
      .filter(Boolean);
    const aLines = String(formatted?.answerSheet || "")
      .split("\n")
      .map((s) => stripNumberPrefix(s))
      .filter(Boolean);
    ...
    const pairs = [];
    for (let i = 0; i < n; i += 1) {
      pairs.push({ q: qLines[i], a: ensureSentenceEnd(titleCaseStart(aLines[i])) });
    }
    return pairs;
  }

  function grammarFocusOf(input = {}) {
    return input?.grammarFocus || detectGrammarFocus(
      [input?.userPrompt, input?.topic, input?.worksheetTitle].filter(Boolean).join(" ")
    );
  }
  ...
  function hasWhatClause(ans) {
    return /^\s*What\b/.test(ans) || /\bwhat\b\s+[A-Za-z][^?!.]*\b(is|was|helped|matters|means|needs|wanted|need|said|learned)\b/i.test(ans);
  }
  ...
  function getAnswerFallbackBank(input = {}) {
    ...
    if (isPresentContinuousTopic(input)) {
      return [
        ...
        "Are they playing outside now?",
        "She is talking with her friend now."
      ];
    }
    ...
    if (isPresentPerfectTopic(input)) {
      return [
        ...
        "She has made a new friend recently."
      ];
    }
  ...
  function diversifyQuestion(q, idx, input = {}) {
    let out = normalizeLine(q);
    if (!out || /^\(?\d+\)?$/.test(out)) {
      out = pickOne(getQuestionTemplateBank(input), idx);
    }
    return out;
  }

  function diversifyAnswer(a, idx, input = {}) {
    const focus = grammarFocusOf(input);
    let out = ensureSentenceEnd(titleCaseStart(a));
    if (!out || !matchesChapter(out, input)) {
      out = pickOne(getAnswerFallbackBank(input), idx);
    }
    ...
    if (focus.chapterKey === "passive") {
      ...
      out = out.replace(/^(This|The)\s+\w+/i, pickOne(subjects, idx));
    }

    if (isPresentContinuousTopic(input)) {
      const starts = ["I am", "She is", "They are", "We are", "He is"];
      out = out.replace(/^(I am|She is|They are|We are|He is)\b/i, pickOne(starts, idx));
    }

    if (isPresentPerfectTopic(input)) {
      const starts = ["I have", "She has", "We have", "He has", "They have"];
      out = out.replace(/^(I have|She has|We have|He has|They have)\b/i, pickOne(starts, idx));
    }

    return ensureSentenceEnd(titleCaseStart(out));
  }

  const __prevFormatMagicResponse_v892 = typeof formatMagicResponse === "function" ? formatMagicResponse : null;

  formatMagicResponse = function formatMagicResponse_v892(rawText, input = {}) {
    const base = __prevFormatMagicResponse_v892
      ? __prevFormatMagicResponse_v892(rawText, input)
      : {
          title: "",
          instructions: "",
          questions: String(rawText || ""),
    ...
    let pairs = toPairArray(base);

    const usedAnswers = new Set();
    const finalPairs = [];

    for (let i = 0; i < pairs.length; i += 1) {
      const q = diversifyQuestion(pairs[i].q, i, input);
      let a = diversifyAnswer(pairs[i].a, i, input);

      if (!matchesChapter(a, input)) {
        a = diversifyAnswer(pickOne(getAnswerFallbackBank(input), i), i, input);
      }

      const key = normalizeLine(a).toLowerCase();
      if (key && !usedAnswers.has(key)) {
        usedAnswers.add(key);
        finalPairs.push({ q, a });
      }
    }
    ...
    while (finalPairs.length < 25) {
      const q = diversifyQuestion(pickOne(qBank, cursor), cursor, input);
      const a = diversifyAnswer(pickOne(aBank, cursor), cursor, input);
      const key = normalizeLine(a).toLowerCase();
    ...
    while (finalPairs.length < 25) {
      finalPairs.push({
        q: `${diversifyQuestion(pickOne(qBank, cursor), cursor, input)} (${finalPairs.length + 1})`,
        a: diversifyAnswer(pickOne(aBank, cursor), cursor, input),
      });
      cursor += 1;
    }
    ...
    base.questions = finalPairs
      .map((p, i) => `${i + 1}. ${stripNumberPrefix(p.q)}`)
      .join("\n");
    base.answerSheet = finalPairs
      .map((p, i) => `${i + 1}. ${stripNumberPrefix(ensureSentenceEnd(titleCaseStart(p.a)))}`)
      .join("\n");
    base.actualCount = 25;
    base.fullText = [base.title, base.instructions, base.questions, input?.language === "en" ? "Answers" : "정답", base.answerSheet]
      .filter(Boolean)
      .join("\n\n");
